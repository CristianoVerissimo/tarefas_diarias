...
def ao_clicar_entrada(event):
    if entrada_tarefa.get() == "Inserir Tarefa":
        entrada_tarefa.delete(0, tk.END)
...
def ao_sair_foco(event):
    if not entrada_tarefa.get().strip():
        entrada_tarefa.delete(0, tk.END)
        entrada_tarefa.insert(0, "Inserir Tarefa")
        entrada_tarefa.configure(fg="grey")

# Carregar ícones (assegure-se que os arquivos estão na mesma pasta que o script)
icon_editar = PhotoImage(file="images/edit.png").subsample(3, 3)  # Ajuste conforme necessário
icon_deletar = PhotoImage(file="images/delete.png").subsample(3, 3)  # Ajuste conforme necessário
...
rotulo_cabecalho = tk.Label(janela, text="Tarefas Diarias", font=fonte_cabecalho, bg="#F0F0F0", fg="#000")
...
botao_adicionar = tk.Button(frame, text="Adicionar", command=adicionar_tarefa, bg="#4CAF50", fg="white", height=1, width=15, font=("Roboto", 11), relief=tk.FLAT)
